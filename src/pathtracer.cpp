...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  // COMMENT OUT `normal_shading` IN `est_radiance_global_illumination` BEFORE YOU BEGIN
    ...
    for(int i = 0; i < num_samples; i ++){
        ...
        Ray cast(hit_p + EPS_D * wi, wi);
        Intersection hit;
        if(bvh->intersect(cast, &hit)){
            Spectrum b = isect.bsdf->f(w_out,w_in);
            Spectrum c = hit.bsdf->get_emission();
            L_out += b * c * cos_theta(w_in);
        }
    }
...
Spectrum PathTracer::estimate_direct_lighting_importance(const Ray& r, const Intersection& isect) {
  ...
  // COMMENT OUT `normal_shading` IN `est_radiance_global_illumination` BEFORE YOU BEGIN
    ...
    for(SceneLight* s: scene->lights){
        ...
        float dist;
        float pdf;
        int step = ns_area_light;
        if(s->is_delta_light())
            step = 1;
        
        Spectrum L;
        for(int i = 0; i < step; i ++){
            Spectrum ss = s->sample_L(hit_p, &wi, &dist, &pdf);
            Vector3D w_in = w2o * wi;
            if(w_in.z >= 0.0){
                Ray rr(hit_p + EPS_D * wi, wi);
                rr.max_t = dist;
                
                Intersection inter;
                if(!bvh->intersect(rr, &inter)){
                    Spectrum b = isect.bsdf->f(w_out, w_in);
                    Spectrum c = (ss * b * cos_theta(w_in) )/ pdf;
                    L += c;
                }
            }
...
Spectrum PathTracer::at_least_one_bounce_radiance(const Ray&r, const Intersection& isect) {
  ...
  // traced ray (when applicable) goes
    ...
    if(r.depth > 1 && coin_flip(cp)){
        ...
        Vector3D wi = o2w * w_in;
        int d = r.depth - 1;
        Ray rr(hit_p + EPS_D * wi, wi, d);
        if(bvh->intersect(rr, &inter)){
            L_out += at_least_one_bounce_radiance(rr, inter) * s * cos_theta(w_in) / pdf / cp;
        }
    }
...
//  return Spectrum();
    ...
    Spectrum spec;
    if(num_samples == 1){
        double sampleX = origin.x + 0.5;
        double sampleY = origin.y + 0.5;
        sampleX /= (double)sampleBuffer.w;
        sampleY /= (double)sampleBuffer.h;
        Ray r = camera->generate_ray(sampleX, sampleY);
        spec += est_radiance_global_illumination(r);
    }else{
        double s1 = 0, s2 = 0;
        int count = 0;
        ...
        for(int i = 0; i < num_samples; i ++){
            ...
            double sampleX = origin.x + rand.x;
            double sampleY = origin.y + rand.y;
            sampleX /= (double)sampleBuffer.w;
            sampleY /= (double)sampleBuffer.h;
            Ray r = camera->generate_ray(sampleX, sampleY);
            r.depth = max_ray_depth;
            Spectrum rr = est_radiance_global_illumination(r);
            spec += rr;
            
            double xk = rr.illum();
            s1 += xk;
            s2 += xk * xk;
            double div = 1.0 / (double)(i + 1);
            
            if(count == samplesPerBatch){
                double u = s1 * div;
                double var = 1.0 / (double)i * (s2 - (s1 * s1)) * div;
                double conv = 1.96 * sqrt(var) / sqrt(i + 1);
                count = 0;
                
                if(conv <= maxTolerance * u){
                    spec /= (float)(i + 1);
                    sampleCountBuffer[x + y * frameBuffer.w] = i + 1;
                    return spec;
                }
            }
        }
